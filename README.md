...
- **电源与电池**: 5V 稳压供电，建议使用自带 XH2.54 插头的一体式充电口/电池组合，保持插头与主板供电插座对齐便于连接与拆装；型号可选购支持 XH2.54 插头的电源模块或电池盒，常见一体式组合可参考淘宝链接 https://e.tb.cn/h.7oMTV0WlAiWJFYN?tk=pgqNUgS4EwD。
...
`Effect_Tick()` 先读编码器加速度与原始增量，再处理单击循环模式、旋转调参数，以及长按时的 Flash 保存流程；接着是模式 7 的全亮降档保护、基于当前参数更新位置/闪烁/呼吸计时，最后根据模式生成 `cmd` 缓冲并写入驱动（参考 `Core/Src/app/app.c:130-416`）。
