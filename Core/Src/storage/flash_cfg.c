...
/**
  ...
  ******************************************************************************
  *
  * Record layout (aligned, append-only):
  * [0..3]   magic (0x31474643 'CFG1')
  * [4..5]   version
  * [6..7]   length (payload bytes)
  * [8..11]  seq (monotonic)
  * [12..(N-5)] payload (Config, padded with 0xFF)
  * [(N-4)..(N-1)] crc32 over bytes [0..(N-5)]
  *
  * Commit mechanism:
...
#define CFG_RECORD_SIZE 80U
#define CFG_HEADER_SIZE 12U
#define CFG_CRC_OFFSET (CFG_RECORD_SIZE - 4U)
#define CFG_SLOTS_PER_PAGE (CFG_PAGE_SIZE / CFG_RECORD_SIZE)

_Static_assert((CFG_RECORD_SIZE % 8U) == 0U, "Record size must be 8-byte aligned");
